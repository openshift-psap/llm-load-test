...
plugin_options:
  ...
  url: "https://<endpoint URI>/v1/chat/completions/"
  model_name: "<Model Name like Phi3>"
  key: "<key>"
"""

required_args = ["key","url", "streaming"]
...
class AzureServerlessPlugin(plugin.Plugin):
    ...
    def request_http(self, query: dict, user_id: int, test_end_time: float = 0):
        ...
        headers = {"Content-Type": "application/json",'x-ms-useragent': 'azure-loadtest' }
    ...
    def streaming_request_http(self, query: dict, user_id: int, test_end_time: float):
        headers = {"Content-Type": "application/json", 'Authorization':('Bearer '+ self.key), 'api-key': self.key, 'x-ms-useragent': 'azure-loadtest' }
            ...
            if found and data != b"[DONE]":
                try:
                    ...
                    if len(message["choices"])==0:
                        message["choices"].append({"delta":{"content":"","role":"assistant"}})
                    if "/chat/completions" in self.host and not message["choices"][0]['delta'].get('content'):

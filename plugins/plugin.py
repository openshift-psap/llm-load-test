class Plugin:
    ...
    def _calculate_results(
        self, start_time, end_time, response, num_tokens, user_id, query
    ):
    ...
    def _calculate_results_stream(
        self, start_time, ack_time, first_token_time, end_time, tokens, user_id, query
    ):
        resp_time = end_time - start_time
        tt_ack = 1000 * (ack_time - start_time)
        ttft = 1000 * (first_token_time - start_time)  # Time to first token in ms
        tpot = (1000 * resp_time - tt_ack) / (
            len(tokens) - 1
